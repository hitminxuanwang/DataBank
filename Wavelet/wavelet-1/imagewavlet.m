clear;clc;close all;
src=im2double(imread('TV_building.bmp'));
dst=im2double(imread('TV_fire.bmp'));
Rs=zeros(size(src));
for k=1:3
[Cs,Ss]=wavedec2(src(:,:,k),3,'db3');
[Cd,Sd]=wavedec2(dst(:,:,k),3,'db3');
fun=@(x) sum(x(:))/numel(x);
...
C=[reshape(cA3,[1,Sd(2,1)*Sd(2,2)]),reshape(cH3,[1,Sd(2,1)*Sd(2,2)]),reshape(cV3,[1,Sd(2,1)*Sd(2,2)]),reshape(cD3,[1,Sd(2,1)*Sd(2,2)]) ...
    reshape(cH2,[1,Sd(3,1)*Sd(3,2)]),reshape(cV2,[1,Sd(3,1)*Sd(3,2)]),reshape(cD2,[1,Sd(3,1)*Sd(3,2)]) ...
    reshape(cH1,[1,Sd(4,1)*Sd(4,2)]),reshape(cV1,[1,Sd(4,1)*Sd(4,2)]),reshape(cD1,[1,Sd(4,1)*Sd(4,2)])];
